...
function ENT:Initialize()
    ...
    self.LastProgressPos = self:GetPos()
    self.LastProgressTime = CurTime()
...
function ENT:TrackProgress()
    local curTime = CurTime()
    local curPos = self:GetPos()

    if curPos:DistToSqr(self.LastProgressPos or curPos) >= (self.ProgressDistance * self.ProgressDistance) then
        self.LastProgressPos = curPos
        self.LastProgressTime = curTime
        return false
    end

    if (self.LastProgressTime or curTime) + self.StuckTimeout <= curTime then
        return true
    end
...
function ENT:ResolveStuck()
    local targetPos = IsValid(self.TargetFire) and self.TargetFire:GetPos() or (self:GetPos() + VectorRand() * 500)
    local dir = (targetPos - self:GetPos()):GetNormalized()
    if dir:LengthSqr() <= 0 then
        dir = VectorRand():GetNormalized()
    end

    local escapePos = self:GetPos() + dir * 180 + VectorRand() * 80 + Vector(0, 0, 36)
    local tr = util.TraceHull({
    ...
    if tr.Hit then
        escapePos = tr.HitPos + tr.HitNormal * 24 + Vector(0, 0, 12)
    end

    self:SetPos(escapePos)
    self.LastProgressPos = self:GetPos()
    self.LastProgressTime = CurTime()
end
